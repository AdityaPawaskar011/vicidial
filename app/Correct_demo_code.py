...
def get_calls_by_status(current_user: str = Depends(get_current_user), campaign_id: str = None):
    try:
        conn   = mysql.connector.connect(**DB_CONFIG)
        cursor = conn.cursor(dictionary=True)
...
def get_dialerperformance(request: Request, current_user: str = Depends(get_current_user)):
    try:
        conn        = mysql.connector.connect(**DB_CONFIG)
        cursor      = conn.cursor(dictionary=True)
        today_start = date.today()

        # Admin only - optional campaign filter from query params
        campaign_id     = request.query_params.get("campaign_id")
        campaign_filter = f"AND campaign_id = '{campaign_id}'" if campaign_id else ""
        ...
        query = f"""
            ...
            WHERE DATE(vl.call_date) = %s {campaign_filter}
        """
